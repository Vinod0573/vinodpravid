...
const ASRIssue = (props) => {
  ...
  const [ASRIssuesToBe, setASRIssuesToBe] = useState([
    "No Transcription",
  ...
  return (
    <>
      <div
        className={`ASRIssueWrapper ${
          props.hideFilterList?.includes("ASRIssue")
            ? "hidePointerEventUniversaljp"
            : ""
        }`}
      >
        {ASRIssuesSelected && ASRIssuesSelected.length > 0 && (
          <div className="placeholder-up">ASR</div>
        )}
        <div>
        ...
        <div className="goinbox">
          {ASRIssuesSelected?.length == 1 ? (
            ...
            <p className="err">
              ...
              >{`+${ASRIssuesSelected.length - 2}`}</span>
            </p>
          ) : (
            <>{``}</>
          )}
