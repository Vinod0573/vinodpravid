...
const NLUIssue = (props) => {
  ...
  const [NLUIssuesToBe, setNLUIssuesToBe] = useState([
    "Missed Entity",
    "Wrong Entity",
    "No Response",
  ...
  return (
    <>
      <div
        className={`NLUIssueWrapper ${
          props.hideFilterList?.includes("NLUIssue")
            ? "hidePointerEventUniversaljp"
            : ""
        }`}
      >
        {NLUIssuesSelected && NLUIssuesSelected.length > 0 && (
          <div className="placeholder-up">NLU</div>
        )}
        <div>
        ...
        <div className="goinbox">
          {NLUIssuesSelected?.length == 1 ? (
            ...
            <p className="err">
              ...
              >{`+${NLUIssuesSelected.length - 2}`}</span>
            </p>
          ) : (
            <>{``}</>
          )}
