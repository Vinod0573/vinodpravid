...
const CustomerIssue = (props) => {
  ...
  const [customerIssuesToBe, setCustomerIssuesToBe] = useState(['No Response','Wrong Response', 'Interrupt', 'Background Noise']);
  ...
  return (
    <>
      <div className={`CustomerIssueWrapper ${props.hideFilterList?.includes('CustomerIssue') ?"hidePointerEventUniversaljp":""}`}>
      {customerIssuesSelected&&customerIssuesSelected.length>0&&<div className="placeholder-up">CUSTOMER</div>}
        <div>
        ...
        <div className="goinbox">
        {
          ...
          <p className="err"><span className="err-box">{customerIssuesSelected[0]}</span></p>:
          (customerIssuesSelected?.length==2)?
          
          <p className="err"><span className="err-box">{customerIssuesSelected[0]}</span>
          <span  className="err-box">{customerIssuesSelected[1]}</span></p>
          :(customerIssuesSelected?.length>2)?
          ...
          <span className="err-boxx" onClick={()=>setIsDropDownShow(true)}>{`+${customerIssuesSelected.length-2}`}</span>
          </p>:
          <>{``}</>
        }
        </div>
...
const mapStateToProps = (state, ownProps) => {
  return {
    customerIssueData: state.reportIssueReducer.customerIssueData,
