...
const OtherIssue = (props) => {
  ...
  return (
    <>
      <div className={`OtherIssueWrapper ${props.hideFilterList?.includes('OtherIssue') ?"hidePointerEventUniversaljp":""}`}>
      {otherIssuesSelected&&otherIssuesSelected.length>0&&<div className="placeholder-up">OTHER</div>}
        <div>
        ...
        <div className="goinbox">{
          ...
          <p className="err"><span className="err-box">{otherIssuesSelected[0]}</span></p>:
          (otherIssuesSelected?.length==2)?
          
          <p className="err"><span className="err-box">{otherIssuesSelected[0]}</span>
          <span  className="err-box">{otherIssuesSelected[1]}</span></p>
          :(otherIssuesSelected?.length>2)?
          ...
          <span className="err-boxx" onClick={()=>setIsDropDownShow(true)}>{`+${otherIssuesSelected.length-2}`}</span>
          </p>:
          <>{``}</>
        }</div>
      </div>
...
const mapStateToProps = (state, ownProps) => {
  return {
    otherIssueData: state.reportIssueReducer.otherIssueData,
