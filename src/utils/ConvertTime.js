...
export const toConvertCalendarDate = (date) => {
  const requireformat =
    date.toString().substring(7, 10) +
    " " +
    date.toString().substring(3, 7) +
    " " +
    " " +
    date.toString().substring(13, 15);
  return requireformat;
};
export const getDayInCorrectFormat = (data) => {
  // console.log(data,"DATAT")
  if (data === undefined) return "";
  const startDate = data.startDate + "";
  const endDate = data.endDate + "";

  if (endDate.substring(11, 15) == startDate.substring(11, 15)) {
    return (
      startDate.substring(4, 10) +
      " - " +
